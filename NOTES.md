...
## [15:15] Bankruptcy logic confirmed correct (not fixed); customizable starting cash added

- **Context:** Two asks. (1) Re-verify the bankruptcy rule (`cash <= 0 AND can't make one more cup`, evaluated once post-melt) actually matches the code, including a specific claim: since ice always melts to 0 and `RECIPE.icePerCup > 0`, the inventory half of that AND is *always* true post-melt regardless of leftover cups/lemons/sugar — meaning the practical trigger reduces to "cash <= 0." (2) Let a player set a custom starting cash instead of the hardcoded `STARTING_CASH`.
- **Options considered:** For starting cash: (a) add the `startingCash` override to `resetGame()` (the function the "start new game"/"exit" routes both already call) vs. (b) add it to `getOrCreateGame()`, since its two creation branches (`!existing` and `existing.isGameOver`) are what actually call `defaultGameStateData()` and set `cash`. For invalid input: (a) reject with a 400/ZodError vs. (b) silently fall back to `STARTING_CASH`.
- **Chose:** Part 1: no code change to the bankruptcy logic itself — confirmed via throwaway-then-permanent tests that it already matches the spec exactly, both halves of the AND are genuinely required (not silently cash-only or inventory-only), and enforcement (`game.isGameOver` checks in `buyInventory`/`setPriceAndSimulateDay`, translated to a 400 by both routes) and the UI's full-screen game-over view were already correct. Part 2: (b) `getOrCreateGame(startingCash?)`, validated with `z.number().positive().max(10_000)` via `safeParse`, falling back silently to `STARTING_CASH` on any failure.
- **Why:** Part 1's claim was proven, not assumed: `maxSellableByInventory({iceStock: 0, ...}, RECIPE)` returned `0` even with 1,000,000 leftover cups/lemons/sugar in a direct test — confirming bankruptcy in this design is, in practice, driven entirely by cash once a day resolves. `resetGame()` is a pure full-wipe with no creation logic at all (by earlier design, since it's shared with "exit"); `getOrCreateGame()` is the only place a GameState row (and its `cash` field) actually gets created, so that's where the override belongs. Falling back silently on invalid `startingCash` (rather than 400ing) matches this field's low stakes — it's not a security boundary, just a guard against a broken/silly game state, so failing the whole "start game" request over a bad number would be worse UX than just using the default.
- **Trade-off / what I gave up:** None for Part 1 — added 3 permanent regression tests (`maxSellableByInventory` is always 0 post-melt; cash≤0 + big leftover inventory ⇒ bankrupt; cash>0 + zero inventory ⇒ not bankrupt) so this can't silently regress. For Part 2, the landing page's starting-cash input has no persistence layer feeding its initial value, so it naturally resets to the default on every fresh mount (including after Exit Game) without any extra code — verified live in the browser: set $250 → game showed $250.00 cash → Exit Game → landing page showed $20 again, not $250.
