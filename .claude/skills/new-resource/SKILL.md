...
Follow the `Item` resource as the template for every step below. Read the
relevant existing file before writing the new one so naming/style match
exactly.

## Steps

1. **Prisma model** — In `prisma/schema.prisma`, add a model for `$0` next to
   (or replacing) `Item`, with the requested fields plus `id`, `createdAt`,
   `updatedAt` following the same shape as `Item`. Run `npm run db:push` to
   sync the schema.

2. **Service layer** — Create `src/services/<entity>Service.ts` modeled
   exactly on `src/services/itemService.ts`:
